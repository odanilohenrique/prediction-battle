...
contract PredictionBattleV7_SECURE is ReentrancyGuard, Pausable, Ownable2Step {
    ...
    // Constants
    // Constants
    uint256 public constant DISPUTE_BLOCKS = 21600; // 12h @ 2s/block (Base L2)
    ...
    function placeBet(
        ...
        // Calcular shares (com bônus apenas no período inicial)
        bool isEarlyBird = block.number < m.creationBlock + m.bonusDuration;
    ...
    function resolveDispute(
        ...
        // M1 Fix: Reward comes from Pool, not House Balance
        // This ensures reporter is always paid.
        totalBond += reward;
        
        // NOTE: We don't interact with houseBalance here anymore for rewards.
        // The pool effectively shrinks by 'reward', so users claim less.
    ...
    function claimWinnings(string calldata _marketId) external nonReentrant {
        ...
        if (m.isVoid) {
            ...
            if (winningBet.amount > 0 && winningBet.shares > 0) {
                ...
                // Deduct reporter reward from pool if paid
                uint256 reporterReward = (totalPool * REPORTER_REWARD_BPS) / FEE_DENOMINATOR;

                // Solvency Fix M1: Distributable is strictly Pool - Fees - ReporterReward
                uint256 totalFees = (totalPool * (houseFeeBps + creatorFeeBps + referrerFeeBps)) / FEE_DENOMINATOR;
                
                uint256 distributablePool = totalPool - totalFees - reporterReward; 
    ...
    /**
     * @notice Retorna bond necessário para um mercado (View)
     */
    function getRequiredBond(string calldata _marketId) external view returns (uint256) {
        Market storage m = markets[_marketId];
        return _getRequiredBond(m.totalYes + m.totalNo);
    }
    
    // ============ INTERNAL FUNCTIONS ============
    
    function _getRequiredBond(uint256 poolSize) internal pure returns (uint256) {
        uint256 dynamicBond = (poolSize * 100) / 10000; // 1%
        return dynamicBond > MIN_BOND ? dynamicBond : MIN_BOND;
    }
    
    /**
     * @notice Calcula shares baseado no ratio do pool
     * @param isEarlyBird Se true, aplica bônus de peso (1.0x-1.5x). Se false, peso fixo 1.0x.
     */
    function _calculateShares(
        ...
        uint256 weight = MIN_WEIGHT; // Base: 1.0x (100)
        
        // Bônus de peso apenas para Early Birds
        if (isEarlyBird) {
            uint256 ratio = (oppositePool * SHARE_PRECISION) / targetPool;
            weight = (ratio * 100) / SHARE_PRECISION;
